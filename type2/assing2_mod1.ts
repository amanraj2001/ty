...
var emp :{id:number,firstname:string,lastname:string,address:string,salary:number}[]=[{"id":6,"firstname":"aman","lastname":"rajput","address":"13/122,dawyers street,pondicherry,India","salary":1500}]
...
function show(){
    ...
    for(let i =0;i<arr.length;i++){
        ...
        result+=`<tr><td>${arr[i].id}</td><td>${arr[i].firstname} ${arr[i].lastname}</td><td>${arr1[i].x1}</td><td>${arr1[i].x2}</td><td>${arr1[i].x3}</td><td>${arr1[i].x4}</td><td>${pf}</td><td>${tsalary}</td><td>${arr[i].salary}</td></tr>`
    }
